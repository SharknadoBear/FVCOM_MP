% EXTRACT_SPATIAL_SNAPSHOTS_A
% Extract spatial snapshot and station time-series fields from the case-a
% FVCOM-MP NetCDF output for distributional checking (Memo 05).
%
...
%   <test_root>/OUTPUT_a/backup_data/waterPACT_a_0001.nc
%
% Default MAT output:
%   <test_root>/OUTPUT_a/spatial_snapshots_a.mat  (v7.3 / HDF5)
%
% HPC usage (run from any directory on Kestrel data-analysis node):
%   cd /scratch/yhuang168/waterPACT_MP_floc
%   matlab -nodisplay -batch "run('MATLAB/extract_spatial_snapshots_a.m')"
%
...
nc_file       = '/kfs3/scratch/yhuang168/waterPACT_MP_floc/OUTPUT_a/backup_data/waterPACT_a_0001.nc';
out_mat       = '/kfs3/scratch/yhuang168/waterPACT_MP_floc/OUTPUT_a/spatial_snapshots_a.mat';
last_n_hours  = 24;
...
if isempty(out_mat)
    out_mat = fullfile(output_dir, ['spatial_snapshots_' case_id '.mat']);
end
...
% --------------------------------------------------------------------------
all_time = double(ncread(nc_file, 'time'));   % days since 0.0
n_total  = numel(all_time);

t_end       = all_time(end);
t_threshold = t_end - last_n_hours / 24.0;
snap_idx    = find(all_time >= t_threshold);

if isempty(snap_idx)
    error('No records found in last %d hours (t_end = %.4f days).', last_n_hours, t_end);
end
n_snaps = numel(snap_idx);
fprintf('Snapshot window: last %d hours -> %d records (%.4f .. %.4f days)\n', ...
    last_n_hours, n_snaps, all_time(snap_idx(1)), all_time(snap_idx(end)));

%% -------------------------------------------------------------------------
%  Allocate snapshot arrays
% --------------------------------------------------------------------------
snap_time    = all_time(snap_idx);            % (n_snaps,)
