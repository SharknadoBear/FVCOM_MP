% extract_mp1_mass_budget_b_c_90day
% Integrate FVCOM-MP floc-case mp1 mass diagnostics for experiments b and c
% across three consecutive output files (_0001 through _0009) to cover a
% ~90-day period.
%
...
%   matlab -batch "run('extract_mp1_mass_budget_b_c_90day.m')"
%
...
if isempty(out_mat)
    out_mat = fullfile(default_output_dir, ['mp1_mass_budget_' strjoin(case_ids(:).', '_') '_90day.mat']);
end
...
for icase = 1:numel(case_ids)
    ...
    if config.save_case_mats
        ...
        case_mat_files{icase} = fullfile(case_output_dir, ...
            ['mp1_mass_budget_' case_id '_90day.mat']);
...
fprintf('  Reading %d NetCDF file(s) (~90-day concatenation).\n', numel(files));
