...
%   matlab -batch "run('extract_mp1_mass_budget_b_c.m')"
%
...
if isempty(out_mat)
    out_mat = fullfile(default_output_dir, ['mp1_mass_budget_' strjoin(case_ids(:).', '_') '.mat']);
end
...
for icase = 1:numel(case_ids)
    ...
    case_nc_file = resolve_nc_file(case_output_dir, char(nc_files{icase}), case_id);

    fprintf('Case %s\n', case_id);
    fprintf('  NetCDF directory: %s\n', case_output_dir);
    fprintf('  Target NetCDF file: %s\n', case_nc_file);

    diagnostics{icase} = extract_one_case(case_id, case_output_dir, case_nc_file, ...
...
%% Local Functions
function diagnostic = extract_one_case(case_id, output_dir, nc_file, time_stride, save_grid_metrics, cap_threshold_kgm3, cap_tolerance_kgm3)
if ~isfile(nc_file)
    error('Target NetCDF file does not exist: %s', nc_file);
end
files = {nc_file};

fprintf('  Reading one NetCDF file.\n');
