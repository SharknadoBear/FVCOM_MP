% EXTRACT_SPATIAL_SNAPSHOTS_B_C
% Extract spatial snapshot and station time-series fields from the case-b
% and case-c FVCOM-MP NetCDF outputs for distributional checking (Memo 05).
%
% For each node and each of the last 24 hourly records this script saves:
...
%   <test_root>/OUTPUT_b/backup_data/waterPACT_b_0001.nc
%   <test_root>/OUTPUT_c/backup_data/waterPACT_c_0001.nc
%
% Default MAT outputs:
%   <test_root>/OUTPUT_b/spatial_snapshots_b.mat  (v7.3 / HDF5)
%   <test_root>/OUTPUT_c/spatial_snapshots_c.mat  (v7.3 / HDF5)
%
% HPC usage (run from any directory on Kestrel data-analysis node):
%   cd /scratch/yhuang168/waterPACT_MP_floc
%   matlab -nodisplay -batch "run('MATLAB/extract_spatial_snapshots_b_c.m')"
%
...
%   WATERPACT_LAST_NHOURS   Number of hours at the end of the run to extract.
%                           Default: 24
...
nc_files     = { ...
    '/kfs3/scratch/yhuang168/waterPACT_MP_floc/OUTPUT_b/backup_data/waterPACT_b_0001.nc'; ...
    '/kfs3/scratch/yhuang168/waterPACT_MP_floc/OUTPUT_c/backup_data/waterPACT_c_0001.nc' };
last_n_hours = 24;
...
%  Loop over cases
% --------------------------------------------------------------------------
fprintf('Starting spatial snapshot extraction for %d case(s).\n\n', numel(case_ids));
...
for icase = 1:numel(case_ids)
    ...
    if isempty(nc_files)
        nc_file = fullfile(case_output_dir, 'backup_data', ...
            sprintf('waterPACT_%s_0001.nc', case_id));
    else
    ...
    out_mat = fullfile(case_output_dir, ['spatial_snapshots_' case_id '.mat']);
    ...
    extract_and_save(case_id, nc_file, out_mat, last_n_hours, ...
...
function extract_and_save(case_id, nc_file, out_mat, last_n_hours, ...
        station_names, station_lat, station_lon, n_stations)
...
%% Select last N hours of output records
all_time = double(ncread(nc_file, 'time'));
n_total  = numel(all_time);
t_end    = all_time(end);
t_thr    = t_end - last_n_hours / 24.0;
snap_idx = find(all_time >= t_thr);

if isempty(snap_idx)
    error('No records in last %d hours (t_end = %.4f days).', last_n_hours, t_end);
end
n_snaps = numel(snap_idx);
snap_time = all_time(snap_idx);

fprintf('  Snapshot window: last %d hrs -> %d records (%.4f .. %.4f days)\n', ...
    last_n_hours, n_snaps, snap_time(1), snap_time(end));
...
%% Read loop -- spatial snapshots (last N hours)
...
out.notes = { ...
    ...
    'Snapshot arrays span the last last_n_hours hours of output.' ...
    };
