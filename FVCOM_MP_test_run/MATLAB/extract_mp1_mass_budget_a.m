% EXTRACT_MP1_MASS_BUDGET
% Integrate FVCOM-MP mp1 water and bed mass from FVCOM NetCDF outputs.
%
% This diagnostic reads one FVCOM output file containing mp1 and bot_mass_mp1,
% computes domain-integrated water-column and bed mass time series, and saves
% a compact MAT file for local analysis. Raw 3-D concentration fields are not
% saved.
%
...
%   matlab -batch "run('extract_mp1_mass_budget_a.m')"
%
...
nc_file = resolve_nc_file(output_dir, nc_file, case_id);

if isempty(out_mat)
    out_mat = fullfile(output_dir, ['mp1_mass_budget_' case_id '.mat']);
end

config = struct();
config.output_dir = char(output_dir);
config.nc_file = char(nc_file);
...
if ~isfile(config.nc_file)
    error('Target NetCDF file does not exist: %s', config.nc_file);
end
files = {config.nc_file};

fprintf('Reading target NetCDF file:\n  %s\n', config.nc_file);
