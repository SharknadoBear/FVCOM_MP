...
\texttt{RIVER\_TS\_SETTING~=~'calculated'} is correctly implemented;
the river source adds a constant $Q\cdot C\cdot\Delta t$ per step to
the true domain mass.  The growing M$_2$ tidal oscillation visible in
the checkpoint-derived mass time series is a sigma-coordinate
depth-correction artefact: the checkpoint at CP3 uses the post-external-mode
depth~$D^{n+1}$ with the pre-advection concentration~$f^n$, while the
checkpoint at CP4 uses $D^{n+1}$ with the concentration~$f^{n+1}$
that has already been scaled by $D^n/D^{n+1}$.  The resulting apparent
per-step mass change oscillates tidally with amplitude proportional to
the accumulated near-source mass~$M(t)$, explaining both the M$_2$
period and the monotonically growing amplitude.  No code fix is required;
the oscillation is a measurement artefact of the checkpoint placement,
not a mass-conservation error.
