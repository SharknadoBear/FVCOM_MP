...
\begin{itemize}
  ...
  \item \textbf{Possible physical origin of the tidal OBC signal.}
        The Delaware Estuary domain has an open boundary near the connection
        between the Delaware Bay and the Chesapeake--Delaware Canal corridor.
        The tidal prism at this node can be large, exchanging a significant
        water volume --- and whatever MP it carries --- on each half-cycle.
        Schuylkill River MP sources (SR\_1--SR\_3, near Philadelphia) discharge
        well upstream of this boundary; over $\sim$10~days, the river plume
        disperses enough to interact with the OBC tidal exchange, which could
        amplify the per-step mass oscillation as plastic reaches the bay mouth.
        However, the oscillation amplitude appears roughly constant from the
        first reporting step (before the plume could realistically arrive at the
        boundary), suggesting that the signal is primarily driven by the tidal
        exchange of the background concentration field rather than by
        Schuylkill-sourced MP.  This can be confirmed by inspecting the spatial
        concentration field near the OBC nodes at days~3--5 when the plume
        would be expected to approach.
